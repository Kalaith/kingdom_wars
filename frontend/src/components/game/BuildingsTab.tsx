...
const BuildingsTab: React.FC = () => {
  ...
  const getBuildingUpgradeCost = useGameStore(
    (state) => state.getBuildingUpgradeCost
  );
  ...
  const getBuildingCategory = (buildingKey: string): string => {
    if (
      ['goldMine', 'farm', 'lumberMill', 'stoneQuarry'].includes(buildingKey)
    ) {
  ...
  const buildingsByCategory = Object.entries(buildings).reduce(
    (acc, [key, building]) => {
  ...
  return (
    <div className="bg-gray-50 p-6 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        {Object.entries(buildingsByCategory).map(
          ([category, categoryBuildings]) => (
                      ...
                      <div className="mb-3">
                        {building.production && building.level > 0 && (
                          <div className="text-xs text-green-600 font-medium">
                            +
                            {formatNumber(building.production * building.level)}
                            /min production
                          </div>
                        )}
                        {building.defense && building.level > 0 && (
                          <div className="text-xs text-blue-600 font-medium">
                            +{formatNumber(building.defense * building.level)}{' '}
                            defense
                          </div>
                        )}
                        {building.effect && (
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mt-8">
          ...
          <ul className="text-sm text-blue-700 space-y-1">
            <li>
              • Focus on resource buildings early to establish a strong economy
            </li>
            <li>
              • Build military structures to train units and defend your kingdom
            </li>
