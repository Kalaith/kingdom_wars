...
const AttackTab: React.FC = () => {
  ...
  const [selectedTarget, setSelectedTarget] = useState<EnemyKingdom | null>(
    null
  );
  ...
  const armyValues = Object.values(army).filter(
  ...
  const getRecommendation = (
    ...
    if (powerRatio >= 1.5)
      return { text: 'Easy Victory', color: 'text-green-600' };
    if (powerRatio >= 1.2)
      return { text: 'Likely Victory', color: 'text-green-500' };
    if (powerRatio >= 0.8)
      return { text: 'Fair Fight', color: 'text-yellow-600' };
    if (powerRatio >= 0.5)
      return { text: 'Difficult', color: 'text-orange-600' };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="card p-6 mb-6">
          ...
          {!hasArmy ? (
            <div className="text-center py-8 text-slate-500">
              ...
              <p className="text-sm">
                Train some units in the Military tab before attacking!
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="text-center p-4 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <div className="text-xl font-bold text-blue-600">
                  {formatNumber(
                    armyValues.reduce((sum, count) => sum + count, 0)
                  )}
                </div>
              </div>
        ...
        {hasArmy && (
          <div className="card p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {gameData.enemyKingdoms.map(
                (enemy: EnemyKingdom, index: number) => {
                  const recommendation = getRecommendation(enemy.power);
                  const isBeingAttacked =
                    selectedTarget?.name === enemy.name && attackInProgress;
                      ...
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="text-right">
                          <div className="font-semibold text-gray-700">
                            Power: {formatNumber(enemy.power)}
                          </div>
                          <div
                      ...
                      <div className="mb-4 p-3 bg-gray-50 rounded">
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-2 mt-1">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, (armyPower / enemy.power) * 50)}%`,
                            }}
                          ></div>
                        </div>
                      </div>
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 mt-6">
          ...
          <ul className="text-sm text-red-700 space-y-1">
            ...
            <li>
              • Successful attacks will yield resources from the enemy kingdom
            </li>
